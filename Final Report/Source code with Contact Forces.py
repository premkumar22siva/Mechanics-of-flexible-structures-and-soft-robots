...
def getFreeIndex(isFixed):
  isFixed[0:7] = 0 # First two nodes and one edge is fixed (clamped) for each actuator
  nv = len(isFixed)
  ...
  freeIndex = np.where(all_DOFs == 0)[0]
  return freeIndex
...
def getFb(q, m1, m2, nat_curvature, EI, voronoiRefLen):
  ...
  for c in range(1,ne): # Loop over all the internal nodes
    ...
    dF, dJ = gradEb_hessEb(node0, node1, node2, m1e, m2e, m1f, m2f, nat_curvature, l_k, EI)
...
def plotrod_simple(q, ctime, x, y, z):
    """
    ...
    ax.set_title(f'Position of actuator at t={ctime:.2f}')
...
def set_equal_axis(ax, center, radius):
    """
    Set equal scaling for the axes of a 3D plot and ensure the plot is centered.

    Parameters:
    - ax: The 3D axis object.
    - center: The center of the sphere (numpy array or list of 3 elements).
    - radius: The radius of the sphere (float).
    """
    limits = np.array([center - radius * 1.5, center + radius * 1.5])
...
def animate_three_rods(
    ...
    objRadius=0.065, objCenter=np.array([0, 0, -0.09]),
    filename="three_rods_simulation.mp4", fps=30
):
    """
    Create an animation of three rods' positions over time with a sphere in the center.
    ...
    - objRadius: Radius of the sphere to represent the object being grasped.
    - objCenter: 3-element array specifying the sphere's center.
    ...
    # Plot the sphere once (it's static)
    u = np.linspace(0, 2 * np.pi, 50)
    v = np.linspace(0, np.pi, 50)
    x = objRadius * np.outer(np.cos(u), np.sin(v)) + objCenter[0]
    y = objRadius * np.outer(np.sin(u), np.sin(v)) + objCenter[1]
    z = objRadius * np.outer(np.ones(np.size(u)), np.cos(v)) + objCenter[2]
    ax.plot_surface(x, y, z, color='b', alpha=0.6)

    # Set equal scaling for the axes
    set_equal_axis(ax, objCenter, objRadius)
...
def plot_gripper_with_sphere(rod_positions1, rod_positions2, rod_positions3, objRadius=0.065, objCenter=np.array([0, 0, -0.09])):
    """
    Visualize the gripper's motion over time in 3D with a sphere in the center.
    ...
    fig.update_layout(
        ...
        title="Gripper Motion with Contact Sphere",
        showlegend=True,
    )
...
##############################################
# Checking if the actuator touches the object
##############################################

def isCrossingSphere(a,b,c,radius,center):
  flag = 0

  # Checking if the actuator node is crossing the sphere's boundary
  if ((a - center[0])**2 + (b - center[1])**2 + (c - center[2])**2) <= radius**2:
    flag = 1

  return flag

#################################
# Normal Vector to the object
#################################

def getNormalVector(a,b,c,center):
  # Compute outward normal vectors

  # r = (x - x_center, y - y_center, z - z_center)
  r_x = a - center[0]
  r_y = b - center[1]
  r_z = c - center[2]
...
def objfun(qGuess, q0, u, a1, a2,
           ...
           ctime, # current time
           objRadius, objCenter, # object dimensions
           x, y, z, # object boundary
  ...
  reactionForce = f # f is zero (or close to zero) for free DOFs. Non-zero at fixed DOFs.
  # The non-zero forces on fixed DOFs are the reaction forces

  return q, u, a1Iterate, a2Iterate, reactionForce
...
r0 = 0.02 # cross-sectional radius (a circular cross-section is assumed for the simulation)

# Matrix (numpy ndarray) for the nodes at t=0
nodes = np.zeros((nv, 3))
# For an initial straight rod configuration
if natR == 0: 
  for c in range(nv):
...
# Material parameters
Y = 386.66e3 # Pascals, effective modulus of elasticity of Elastosil MA 4601
#Y = 169.56e3 # Pascals, effective modulus of elasticity for actuator made of Soft Translucent Silicone
G = Y / 3 # shear modulus of elastomers with large-strain elasticity (assuming the elastomer is isotropic in all directions and incompressible)

# Stiffness parameters
EI = Y * np.pi * r0**4 / 4 # Bending stiffness (assumed to be constant in this script)
...
for c in range(nv): # 0, 1, 2, ..., nv-1
  ind = [4*c, 4*c+1, 4*c+2] # The first three DOFs correspond to the node parameters
  if c == 0: # first node
...
# mass at the edges
for c in range(ne):
  massVector[4*c + 3] = 1/2 * dm * r0**2 # Every fourth DOF corresponds to the edge parameter
...
for k in range(nv):
  C[k,k] = 6 * np.pi * visc * r0 # Using Stoke's law assuming the viscous force on discrete spheres

# Dimensions of the spherical object to be grasped
objRadius = 0.065 # m, radius of the sphere
objCenter = np.array([0,0,-0.09]) # m, center of the sphere
...
x = objRadius * np.sin(theta) * np.cos(phi) + objCenter[0]
y = objRadius * np.sin(theta) * np.sin(phi) + objCenter[1]
z = objRadius * np.cos(theta) + objCenter[2]
...
# Plot initial position of Finger 1
plt.figure(1)
plotrod_simple(q0_finger1, 0, x, y, z)

# Plot initial position of Finger 2
plt.figure(2)
plotrod_simple(q0_finger2, 0, x, y, z)

# Plot initial position of Finger 3
plt.figure(3)
plotrod_simple(q0_finger3, 0, x, y, z)
...
# Fixed and Free DOFs
isFixed = np.zeros(nv) # If c-th element of isFixed is 1, then node c is fixed by the boundary of the object being grasped
freeIndex = getFreeIndex(isFixed)
...
P = np.zeros(ndof) # unit normal vector to the object
P_node = np.zeros(3) # unit normal vector of each node touching the object
reaction_force_node = np.zeros(3) # reaction force on the node

for timeStep in range(Nsteps):
  qGuess = q0_finger1.copy()

  # Predictor step
  q, u, a1, a2, reactionForce = objfun(qGuess, # Guess solution
                                       q0_finger1, u, a1, a2,
                                       freeIndex, # Boundary conditions
                                       dt, tol, # time stepping parameters
                                       refTwist, # guess refTwist to compute the new refTwist
  ...
  for c in range(nv): # Loop over each node and check for two conditions
    P[4*c:4*c+3] = getNormalVector(q[4*c], q[4*c+1], q[4*c+2], objCenter)
    P_node = P[4*c:4*c+3]
    reaction_force_node = reactionForce[4*c:4*c+3]
    # Condition: if the coordinates of each node is within the spherical object
    if isFixed[c] == 0 and isCrossingSphere(q[4*c], q[4*c+1], q[4*c+2], objRadius, objCenter) == 1:
      isFixed[c] = 1 # Fix the node that is crossing the object boundary
      qGuess[4*c:4*c+3] = objCenter + objRadius * P_node # Fixing the node to surface of the object boundary
      needCorrector = True
      break
  ...
  u = (q - q0_finger1) / dt # velocity vector
  ...
  # Simulate the motion of finger 1
  plt.figure(4)
  plotrod_simple(q,ctime,x,y,z)
  plt.show()

# Plot the points of contact of finger 1 with the object
...
count = 0 # to count the number of nodes touching the object
totalForce = 0 # sum of reaction forces at the nodes in contact with the object (contact zone)
for c in range(2,nv):
  if isFixed[c] == 1:
    count += 1
    ax.quiver(q[4*c], q[4*c+1], q[4*c+2], P[4*c], P[4*c+1], P[4*c+2], length=0.5, color='r', linewidth=1)
    totalForce += np.linalg.norm(reactionForce[4*c:4*c+3])
...
ax.set_zlabel("Z-axis")
ax.set_box_aspect([1, 1, 1])  # Equal aspect ratio
plt.title("Normal Vectors at points of contact of Finger 1 with the object")
plt.savefig('Points of contact of Finger 1.png')
plt.show()

if count == 0:
  print("\nFinger 1 is not touching the object")
else:
  print(f"\n{count} nodes of Finger 1 are in contact with the object")

print(f"\nTotal reaction force at the contact zone: {totalForce: .2f} N")
...
u = np.zeros_like(q0_finger2) # velocity vector
Nsteps = round(totalTime / dt ) # Total number of steps
ctime = 0 # current time
...
for timeStep in range(Nsteps):
  ...
  u = (q - q0_finger2) / dt # velocity vector
  # Update q0 with the new q
  q0_finger2 = q.copy()
  ...
  # Simulate the motion of finger 2
  plt.figure(6)
  plotrod_simple(q,ctime,x,y,z)
  plt.show()

# Plot the points of contact of finger 2 with the object
...
ax.set_zlabel("Z-axis")
ax.set_box_aspect([1, 1, 1])  # Equal aspect ratio
plt.title("Normal Vectors at points of contact of Finger 2 with the object")
plt.savefig('Points of contact of Finger 2.png')
plt.show()

if count == 0:
  print("\nFinger 2 is not touching the object")
else:
  print(f"\n{count: .2f} nodes of Finger 2 are in contact with the object")
...
u = np.zeros_like(q0_finger3) # velocity vector
Nsteps = round(totalTime / dt ) # Total number of steps
ctime = 0 # current time
...
for timeStep in range(Nsteps):
  ...
  if needCorrector == True:
    freeIndex = getFreeIndex(isFixed)
    q, u, a1, a2, reactionForce = objfun(qGuess, q, u, a1, a2, freeIndex,
                                         dt, tol, refTwist, massVector, mMat, EA,
                                         refLen, EI, GJ, voronoiRefLen, twistBar,
                                         Fg, C, ctime,
                                         objRadius, objCenter,
                                         x, y, z,
                                         max_iter)

  u = (q - q0_finger3) / dt # velocity vector
  # Update q0 with the new q
  q0_finger3 = q.copy()
  ...
  # Simulate motion of finger 3
  plt.figure(8)
  plotrod_simple(q,ctime,x,y,z)
  plt.show()

# Plot the points of contact of finger 3 with the object
...
ax.set_zlabel("Z-axis")
ax.set_box_aspect([1, 1, 1])  # Equal aspect ratio
plt.title("Normal Vectors at points of contact of Finger 3 with the object")
plt.savefig('Points of contact of Finger 3.png')
plt.show()

if count == 0:
  print("\nFinger 3 is not touching the object")
else:
  print(f"\n{count: .2f} nodes of Finger 3 are in contact with the object")
...
import numpy as np
import matplotlib.pyplot as plt  # For the colormap
import plotly.graph_objects as go
...
def plot_gripper_with_sphere(rod_positions1, rod_positions2, rod_positions3, objRadius=0.065, objCenter=np.array([0, 0, -0.09])):
    ...
    # Define a colormap for the rods over time
    colors = np.linspace(0, 1, timesteps)
    cmap = plt.cm.viridis  # Use the Viridis colormap

    # Add the rods
    for t in range(timesteps):
        color = f"rgba({cmap(colors[t])[0] * 255}, {cmap(colors[t])[1] * 255}, {cmap(colors[t])[2] * 255}, 0.7)"

        # Plot each gripper rod
        for rod, name in zip(
            [rod_positions1[t], rod_positions2[t], rod_positions3[t]],
            ["Finger 1", "Finger 2", "Finger 3"]
        ):
    ...
    # Add legend points for start and end of actuation
    start_color = f"rgba({cmap(colors[0])[0] * 255}, {cmap(colors[0])[1] * 255}, {cmap(colors[0])[2] * 255}, 1)"
    end_color = f"rgba({cmap(colors[-1])[0] * 255}, {cmap(colors[-1])[1] * 255}, {cmap(colors[-1])[2] * 255}, 1)"

    fig.add_trace(
        go.Scatter3d(
            x=[None], y=[None], z=[None],  # Dummy trace for legend
            mode="markers",
            marker=dict(size=10, color=start_color),
            name="Start of Actuation"
        )
    )
    fig.add_trace(
        go.Scatter3d(
    ...
    fig.update_layout(
        scene=dict(
            xaxis=dict(title="X", backgroundcolor="white", showgrid=True),
            yaxis=dict(title="Y", backgroundcolor="white", showgrid=True),
            zaxis=dict(title="Z", backgroundcolor="white", showgrid=True),
        ),
        title="Gripper Motion with Contact Sphere",
        showlegend=True,
    )
...
# Call the function with specified sphere parameters
plot_gripper_with_sphere(rod_positions_1, rod_positions_2, rod_positions_3, objRadius=0.065, objCenter=np.array([0, 0, -0.09]))
...
plt.ylabel('z-coord of last node (Finger 1), $\\delta_z$ [m]')
plt.savefig('Tip deflection of Finger 1.png')

plt.figure(9)
time_array = np.arange(1, Nsteps + 1) * dt
plt.plot(time_array, endY, 'bo-')
plt.box(True)
plt.xlabel('Time, t [sec]')
plt.ylabel('y-coord of last node (Finger 1), $\\delta_z$ [m]')
